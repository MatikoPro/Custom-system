...
<div class="container-fluid px-4">
    <div class="card mt-4 shadow-sm">
        ...
        <div class="card-body">
            ...
            $countQuery = "SELECT COUNT(*) as total FROM products WHERE status = '0'";
            $countResult = mysqli_query($conn, $countQuery);
            $productCount = mysqli_fetch_assoc($countResult)['total'];
            $totalPages = ceil($productCount / $limit);

            // Fetch products with limit
            $query = "SELECT * FROM products WHERE status = '0' ORDER BY id DESC LIMIT $start, $limit";
                    ...
                    <table class="table table-striped table-bordered" id="productTable">
                        ...
                        <tbody>
                            ...
                            foreach ($products as $item) : 
                                // Convert expense values to readable text
                                $expenseText = '';
                                switch($item['expense']) {
                                    case 'mikopo':
                                        $expenseText = 'Mikopo';
                                        break;
                                    case 'wafanyakazi':
                                        $expenseText = 'Wafanyakazi';
                                        break;
                                    case 'mengineyo':
                                        $expenseText = 'Mengineyo';
                                        break;
                                    default:
