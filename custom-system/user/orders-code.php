...
if (isset($_POST['saveOrder'])) {
    ...
    foreach ($sessionProducts as $prodItem) {
        ...
        // Get product expense details
        $getProductQuery = mysqli_query($conn, "SELECT expense, expense_percentage FROM products WHERE id='$productId'");
        $productData = mysqli_fetch_assoc($getProductQuery);
        
        // Calculate actual expense amount based on paid amount percentage
        $expensePercentage = $productData['expense_percentage'];
        $expenseAmount = ($totalProductAmount * $expensePercentage) / 100;

        // Insert into new expenses_tracking table
        $expenseData = [
            'order_id' => $lastOrderId,
            'product_id' => $productId,
            'expense_type' => $productData['expense'],
        ...
        insert('expenses_tracking', $expenseData);
